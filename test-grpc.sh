...
grpcurl -plaintext -import-path ./spec -proto service.proto -d '{"name":"test-template0","namespace":"test-ns","image":"node:25-alpine3.21","command":["npx"],"args":["--y","@modelcontextprotocol/server-memory"],"envs":{"LOG":"debug"},"secret_envs":["test-secret00"],"secret_mounts":[{"name":"test-secret01","mount_path":"/secret/test-secret01"}],"resource_limit_name":"m0-nano","labels":{"app":"mcp"}}' 127.0.0.1:3000 mcp.orchestrator.v1.McpOrchestratorService/CreateMcpTemplate
